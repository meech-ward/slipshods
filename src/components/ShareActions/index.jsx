...
export default function ShareActions({ className, url, title }) {
  return (
    <div className={twMerge('flex flex-col justify-center items-center', className)}>
      <button className='flex mb-6 items-center' onClick={() => navigator.clipboard.writeText(url)}><LinkIcon className="h-7 w-7 mr-2" aria-hidden="true" />Copy Link</button>
      <div className="flex w-full justify-between">
        <a
          href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent(url)}`}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center h-8 w-8 rounded-full  text-gray-400 hover:text-gray-500"
        >
